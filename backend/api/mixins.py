...
class AddRemoveMixin:
    ...
    """
    def add_or_remove(self, request, model, serializer_class, add_message, remove_message):
        if not request.user.is_authenticated:
            return Response({"detail": "Please login or create an account."}, status=status.HTTP_401_UNAUTHORIZED)
